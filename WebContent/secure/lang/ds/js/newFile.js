...
var buttonName, lastnodeCount = printfCount = outPutCount = nodeCount = lineCount = searchCount = preCount = 1, iVal = count = 0;
var flag = false;
...
function createDynamicNodes(nodeCount) {
	...
	$("#dynamicNodes").append('<div class="opacity00 col-xs-2 nodes node list-nodes" id="node' + nodeCount 
					...
					+ ' <div id="nodedata' + nodeCount + '"><div id="dataDiv' + nodeCount + '"'
...
function deleteAtPosition() {
	$("#csllMethodOperations").append('<div class="deleteAtEndElement"><span id="deleteAtEndDec">'
			...
			+ '\t\t\t<span id="assnNullTofst">first = NULL;</span>\n'
			+ '\t\t} else {\n'
			+ '\t\t\t<span id="whileLstNotEqFst">while (prev -> next != first) {</span>\n'
			+ '\t\t\t\t<span id="assnlstToPrev">prev = prev->next;</span>\n'
			+ '\t\t\t}\n'
			+ '\t\t\t<span id="firstEqLastNodeNext">first = lastNode->next;</span>\n'
			+ '\t\t\t<span id="assnFstToPrevNxt">prev -> next = first;</span>\n'
			+ '\t\t}\n'
			+ '\t } else {\n'
			...
			+ '\t}\n'
			+ '\t<span id="printf5">printf("The deleted item from Circular Linked List : %d", lastNode -> data);</span>\n'
			+ '\t<span id="freeNode">free(lastNode);</span>\n'
			+ '\t<span id="rtnFirst">return first;</span>\n'
			+ '}</div>');
}
		...
		$('.introjs-helperLayer').one('transitionend', function() {
			switch (elementId) {
				...
				case "buttonDiv" :
					...
					buttonName, lastnodeCount = printfCount = outPutCount = nodeCount = lineCount = searchCount = preCount = 1, iVal = count = 0;
				...
				case "callToDeleteAtEndMethod" :
					...
					switch(animateStep) {
						...
						case "returnFirstVal" :
							$('.introjs-tooltip').removeClass("hide");
							text = 'The <span class="ct-code-b-yellow">deleteAtEnd()</span> method call '
...
function deleteAtEndMethodVariablesDeclaration() { // error
	$(".introjs-duplicate-nextbutton").remove();
	declareNodesWhenFunctionCall("firstNode", "firstNodeVal", "fstNodeInAddMtd", "first<sub>deleteAtEnd</sub>", function() {
		introcode.refresh();
...
/*function iflstNtEqFst() {
	$(".introjs-duplicate-nextbutton").remove();
	arrow("#ifPosEqOne","#iflstNtEqFst");
	ifFrstToNulText("lastNode -> next", "first", function() {
		fromEffectWithTweenMax("#iflstNtEqFst", "#ifCondition", true, function() {
			$('.introjs-tooltipbuttons').append('<a class="introjs-button introjs-duplicate-nextbutton">Next &#8594;</a>');
			$('.introjs-duplicate-nextbutton').click(function () {
				ifConditionInDelAtPos("#first", $("#next" + lastnodeCount).text(), $("#firstNodeVal").text(), 2, "#second");
			});
		});
	});
}
...
function ifEqualCondition(value1, value2) {
	...
	flipEffectWithTweenMax("#first", $(value1).text(), function() {
		flipEffectWithTweenMax("#second", $(value2).text(), function() {
			...
			text = 'The condition evaluates to <span class="ct-code-b-yellow">'+ con +'</span>.';
			typing("#appnedSpan", text, function() {
				if (buttonName == "deleteAtEnd" || buttonName == "deleteAtBegin" || buttonName == "deleteAtPosition" ) {
					if (con) {
	...
	if (buttonName == "deleteAtBegin") {
		...
		if ($('.node').length == 1) {
			...
			TweenMax.to("#node1", 0.5, { top : -80, onComplete: function() {
				...
				deleteAtFirstNode(1, "#node1", function() { // error in this function 
					setTimeout(function() {
						lastnodeCount--;
						changePosIds();
...
function assnFstToPrevNxt() {
	...
	arrow("#whileLstNotEqFst", "#assnFstToPrevNxt");
...
function whileConTrueOrFalseState() {
	...
	if (buttonNames == "deleteAtPosition") {
		coutDiv = preCount;
	} else {
		coutDiv = lastnodeCount;
	}
	var con = $("#next" + preCount).text() != $("#firstNodeVal").text();
	if (con) {
		if (buttonName == "deleteAtBegin") {
			arrow("#whileLstNotEqFst", "#assnlstToPrev");
		} else if (buttonName == "deleteAtEnd") {
