...
<script src="/secure/lang/c/js-min/mri.min.js"></script>

<title>matrix row interchange</title>
...
<div class="col-xs-12 padding00">
	<div class="ct-box-main">
		<div class="text-center">
			<h1 class="label ct-demo-heading text-center" id="headdingSection">Matrix Rows Interchange</h1>
		</div> 
	</div>
...
#include &lt;stdio.h&gt;
void main() {
	<span id="arrDec">int arr[3][3], row, col, row1, row2, temp = 0;</span>
	...
	<span id="printf2">printf("Enter the numbers of two rows to be exchanged\n");</span>
	<span id="scanf2">scanf("%d %d", &row1, &row2);</span>
	<span id="forLoop3">if(row1 >= 0 && row2 >= 0 && row1 < 3 && row2 < 3) {
		for (row = 0; row < 3; row++) {
			temp = arr[row1][row];
			arr[row1][row] = arr[row2][row];
			arr[row2][row] = temp;
		}
	} else {
 		printf("\nNot possible\n");
	}</span>
	<span id="printf3">printf("The matix after interchanging the two rows(in the original matrix\n");</span>
	<span id="forLoop4">for(row = 0; row < 3; row++) {
		...
		<span id="ifCondition">if(<span class="position" id="firstCon"><span class="position" id="row1">row1</span> &gt;= 0</span> && <span class="position" id="secondCon"><span class="position" id="row2">row2</span> &gt;= 0</span> && <span class="position" id="thirdCon"><span class="position" id="row3">row1</span> &lt; 3</span> && <span class="position" id="fourthCon"><span class="position" id="row4">row2</span> &lt; 3</span>) {</span>&emsp;<span id="forTrueOrFalse"></span>
			<span id="forLoop">for (row = <span class="position" id="rowIni">0</span>; <span id="forRowCon"><span class="position" id="rowCon">row</span> < 3</span>; <span id="rowInc">row++</span>) {</span>&emsp;<span id="trueOrFalse"></span>
				<span id="tempValSpan">temp = arr[<span class="position" id="arrRow1">row1</span>][<span class="position" id="arrRowIdx1">row</span>];</span>
				<span id="row1MinVal">arr[<span class="position" id="arrRow11">row1</span>][<span class="position" id="arrRowIdx3">row</span>] = arr[<span class="position" id="arrRow2">row2</span>][<span class="position" id="arrRowIdx2">row</span>];</span>
				<span id="row2MinVal">arr[<span class="position" id="tRow2">row2</span>][<span class="position" id="arrRowIdx4">row</span>] = <span class="position" id="temp">temp</span>;</span>
