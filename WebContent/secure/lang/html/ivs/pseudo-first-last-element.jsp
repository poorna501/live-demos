...
<div id='styleDiv'>
<span id='totalStyle'><input id="selector" class="input-css" maxlength='15'> {
...
&lt;/html&gt;</pre>
	...
	function selectorChecking(pseudoSelector) {
		$('pre input').removeAttr('disabled').focus();
		$('pre input').off("keyup");
		$('pre input').on("keyup", function(e) {
			$('pre input').val($('input').val().trim());
			if ($('pre input').val() == pseudoSelector) {
				if ($('.user-btn').length == 0) {
					$('pre input').attr('disabled','disabled');
	...
	function finalAnimation(classValue) {
		...
		$('#browserText '+$('input').val()).addClass('class-transfer-move');
