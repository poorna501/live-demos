...
<script type="text/javascript">
		...
		intro.onafterchange(function(targetElement) {
			...
			case "notButton":
				...
				$('.introjs-helperLayer ').one('transitionend', function() {
					var text = "<b class='ct-code-b-yellow'>:not()</b> pseudo element can be used to select all the elements which does not"
								+" have any content.";
					typing(".introjs-tooltiptext", text, function() {
						$('#notButton').removeAttr('disabled').addClass('visited z-index');
						intro.refresh();
			...
			case "selector1":
			 ...
			 case "totalStyle1":
					$(".introjs-nextbutton").hide();
					$('.introjs-helperLayer ').one('transitionend', function() {
						var text = "Apply these set of css rules to targated elements.";
						typing(".introjs-tooltiptext", text, function() {
	...
	function selectorChecking(pseudoSelector,inputNumber) {
		$('input:eq('+inputNumber+')').removeAttr('disabled').focus();
		$('input:eq('+inputNumber+')').off("keyup");
		$('input:eq('+inputNumber+')').on("keyup", function(e) {
			if ($('input:eq('+inputNumber+')').val() == pseudoSelector) {
				if ($('.user-btn').length == 0) {
					$('input:eq('+inputNumber+')').attr('disabled','disabled');
					$('.introjs-nextbutton').show();
				}
			}
		});
	...
	function emptyAnimation() {
		$('.user-btn').remove();
		$(".bg-yellow").eq(0).effect("transfer", {to: $("#browserText > div :empty:eq(0)"), className: "ui-effects-transfer"}, 1500, function() {
			$('#browserText > div :empty:eq(0)').animate({height:'20px'},'slow', function() {
				$(".bg-yellow").eq(1).effect("transfer", {to: $("#browserText > div :empty:eq(1)"), className: "ui-effects-transfer"}, 1500, function() {
	...
	function notAnimation() {
		$('.user-btn').remove();
		$(".bg-yellow").eq(0).effect("transfer", {to: $("#browserText h4:eq(0)"), className: "ui-effects-transfer"}, 1500, function() {
			$('#browserText h4:eq(0)').css('color','red');
			$(".bg-yellow").eq(1).effect("transfer", {to: $("#browserText h4:eq(1)"), className: "ui-effects-transfer"}, 1500, function() {
...
<div id='styleDiv'>
<span id='totalStyle'><input id="selector" class="input-css" maxlength='8'> {
	height:&nbsp;20px;
	background:&nbsp;yellow;
}</span>
<span id='totalStyle1'><input id="selector1" class="input-css" maxlength='12'> {
	color:&nbsp;red;
}</span>
</div>
&lt;/style&gt;
